...
def bian(Yong_shu, n=1):
    ...
    print('第%s变:' % n, Yong_shu, end='   ')
    save('第%s变：%s' % (n, Yong_shu), end='   ')
...
def 大衍筮法():
    ...
    #print('> 十八变成卦')
    print('> 六爻(%s) >> 本卦-%s(%s) >> 变卦-%s(%s)' % (yaos, ben_gua, gbin0,  bian_gua, gbin1))
    save('六爻(%s) >> 本卦-%s(%s) >> 变卦-%s(%s)' % (yaos, ben_gua, gbin0, bian_gua, gbin1))
